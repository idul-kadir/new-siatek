...
| 2026-08-11 | `jurusan-tridharma` | **Model data: 1 data = 1 ketua + banyak anggota, dan 1 data = banyak file.** Kolom "Ketua" diganti **"Tim"** berisi chip (ketua = badge gelap `chip-ketua` + ikon `fa-user-tie`, anggota = chip abu `chip-anggota`); kolom "Aksi" → **"File"** berisi **beberapa tombol unduh** per baris (ikon per tipe file: pdf/excel/zip). Berlaku untuk penelitian & pengabdian (tim + 2 file); pendidikan & penunjang tetap kolom "Diunggah oleh" namun file-nya banyak (2/baris). Pencarian `data-cari` kini juga memuat nama semua anggota tim. Verifikasi render: 47 baris, 27 chip ketua (14 penelitian + 13 pengabdian), file pertama 47 + file kedua per baris, `php -l` & render test lolos |
| 2026-08-11 | `jurusan-tridharma` | **Tim 10+ anggota tidak meledak.** Kolom Tim kini menampilkan ketua + 2 anggota, sisanya diringkas tombol **"+N"** (`chip-more`) yang membuka **popover** (`team-popover`, `position:fixed`, tutup saat klik di luar/scroll/Escape) berisi daftar lengkap — nama anggota tetap hidup di HTML (chips), JS hanya menyembunyikan chip ke-3 dst dan meng-clone teksnya untuk popover. Dummy dibuktikan di baris "Sistem Monitoring Kualitas Udara Berbasis IoT" (penelitian) yang kini punya **12 anggota** → tampil ketua + 2 + "+10". `php -l` & render test lolos |
| 2026-08-11 | `jurusan-dok-akademik` | **Layout Dokumen Akademik (frontend-only, dummy di HTML).** 4 kategori — Kurikulum (4), RPS (6), Silabus (5), Lainnya (5) — tile statistik gradien yang bisa diklik pindah tab, tab buttons bersetatus count, toolbar (cari `data-cari` + `select` tahun 2016–2025 + reset), satu tabel header gelap sticky per kategori (kurikulum: nama/prodi/tahun/SK/file; RPS & silabus: matkul/kode/smt/SKS/prodi/tahun/file; lainnya: nama/kategori/tahun/pengunggah/file), zebra + index berwarna per kategori, tombol unduh `.btn-circle` 2 file/baris (pdf/word/excel), pagination **10/halaman** (prev/next + label), empty state, modal **Unggah Dokumen** (dummy — form + toast "disimpan (dummy)"). JS murni tampilan: tab, filter, pagination (`ITEM_PER_PAGE = 10`), modal pakai atribut `data-upload-open/close` (bukan `data-modal-*` milik modal TTD global). `php -l` & render test lolos (20 baris, 4 thead) |
| 2026-08-11 | `jurusan-dok-akademik` | **Layout diganti total — dari tile+tab+tabel (mirip Tridharma) menjadi ACCORDION per kategori.** User menolak template yang sama. Baru: header + toolbar (cari `data-cari` + `select` tahun + reset) + baris ringkasan "Menampilkan N dokumen" + tombol **Buka Semua/Tutup Semua** (`btnExpandAll`), lalu 4 item accordion (`acc-item acc-{warna}`, `--cat` accent bar kiri, chevron, badge count warna kategori). Isi accordion = **daftar baris** (bukan tabel): `.dok-row` = ikon tipe file (`file-pdf/word/excel/img`) + judul + chips meta (prodi/kode/Smt·SKS/SK/kategori/pengunggah/tahun) + tombol unduh `.btn-circle` 2 file + tooltip. Animasi buka/tutup via `grid-template-rows 0fr→1fr`. JS: toggle accordion (aria-expanded), filter cari/tahun hidupkan badge count per kategori + auto buka item yang cocok / tutup yang kosong + `.acc-empty`, reset mengembalikan hanya item pertama terbuka. Modal unggah dummy dipertahankan. `php -l` & render test lolos (20 `dok-row`, 0 `<table`, 0 `tile-stat`) |
| 2026-08-11 | `jurusan-dok-akademik` | **1 data = 1 file.** Per instruksi user, tombol unduh ganda (emerald pdf + sky word/excel) di setiap `.dok-row` diganti **satu tombol label** berisi nama file: `inline-flex ... gap-2 rounded-lg border ...` + ikon tipe file berwarna emerald + nama file (truncate `max-w-[180px]`) + ikon unduh. Ikon kiri `.file-ico` mengikuti tipe file tunggal tersebut. Diterapkan via regex ke 20 baris (20 replace). `php -l` & render test lolos (20 `dok-row`, 20 tombol 1-file, 0 `btn-circle` di baris) |
| 2026-08-11 | `jurusan-dok-akademik` | **Tombol aksi + filter kategori.** (1) Nama file di tombol unduh dihapus — tombol label diganti **grup aksi 3 tombol `.btn-circle`** per baris: Unduh (emerald `fa-download`), **Edit** (sky `fa-pen`), **Hapus** (red `fa-trash`), masing-masing tooltip `Unduh/Edit/Hapus` (tanpa nama file). (2) Toolbar ditambah `<select id="fKategori">` (Semua/Kurikulum/RPS/Silabus/Lainnya); JS `render()` kini `catMatch = kat==='' || kat===cat` → baris item kategori tak cocok disembunyikan (`it.style.display='none'`), reset mengosongkan `fKategori`. Diterapkan regex ke 20 baris. `php -l` & render test lolos (20× Unduh/Edit/Hapus, 0 `max-w-[180px]`, `fKategori` aktif) |
| 2026-08-11 | `jurusan-dok-akademik` | **Modal Edit.** Ditambah `editModal` — duplikat dari `uploadModal` (form sama: nama/kategori/prodi/tahun/semester/file), judul "Edit Dokumen Akademik" (ikon `fa-pen` sky), tombol submit "Simpan Perubahan", close `data-edit-close` (X + Batal). 20 tombol Edit diberi `data-edit-open` (regex, 20 replace). JS: `[data-edit-open]` membuka modal + **mengisi `#inpEditNama` dari judul `.dok-row` yang diklik** (`b.closest('.dok-row') > p.truncate`), `[data-edit-close]` menutup, `#formEdit` submit → toast "Perubahan disimpan (dummy)". `php -l` & render test lolos (1 `editModal`, 21 `data-edit-open` = 20 tombol + 1 selector JS) |
| 2026-08-11 | `jurusan-dok-akademik` | **Pagination per section** (menjawab "kalau dokumen sangat banyak"). Setiap accordion body diberi pager sendiri (`.acc-pager`): tombol `.pg-prev`/`.pg-next` (`.pg-btn`) + label `.pg-label` "Halaman x / y". JS: `PAGE_SIZE = 5`; `renderItem(it)` memfilter baris per item → hitung `totalHal` → clamp `data-page` → tampilkan slice `[mulai, mulai+PAGE_SIZE)`, update badge count (total cocok, bukan per halaman) + label + disabled state tombol. Klik pager hanya `renderItem(it)`; saat filter cari/tahun/kategori berubah `resetPages()` mengembalikan semua ke halaman 1. Demo: RPS (6 baris) → 2 halaman. `php -l` & render test lolos (4 `.acc-pager`, `PAGE_SIZE = 5`) |
| 2026-08-11 | struktur folder | **Folder `pages/jurusan/dokumen/{akademik,akreditasi,lkps}` dipindah langsung di bawah `jurusan`** → `pages/jurusan/{akademik,akreditasi,lkps}` (user minta struktur tidak dikelompokkan dalam folder `dokumen`). File + guard `index.php` ikut dipindah, folder `dokumen/` dihapus. Route map `index.php` diupdate: `'jurusan/dokumen/akademik'` → `'jurusan/akademik'`, dst. URL slug & sidebar **tidak berubah** (`/jurusan-dok-akademik` dst). Render test via router 3 halaman lolos (0 404/fatal) |
